...
(function () {
  ...
  async function fetchTrendingVideos(limit = 10) {
    const { data, error } = await window.sb.from('posts')
      .select('id,title,media_url,media_type,views,profiles:user_id(display_name,avatar_url)')
      .eq('media_type', 'video').order('views', { ascending: false }).limit(limit);
    return error ? [] : (data || []);
  }
  ...
  async function fetchTrendingPhotos(limit = 9) {
    const { data, error } = await window.sb.from('posts')
      ...
      .eq('media_type', 'image').order('views', { ascending: false }).limit(limit);
    return error ? [] : (data || []);
  }
  ...
  async function searchAll(q) {
    ...
    const [postsRes, usersRes, marketRes] = await Promise.all([
      window.sb.from('posts').select('*, profiles:user_id(id,display_name,username,avatar_url,verified_status)')
        .or(`title.ilike.${like},content.ilike.${like}`).order('created_at', { ascending: false }).limit(30),
  ...
  // ─── SQUARE (hashtag mode + photos tab) ────────────────────
  ...
  // ─── FEED-STYLE POST CARD (Top tab — matches home feed, not a text card) ──
  function feedStylePostCard(p) {
    const profile = p.profiles || p.profile || {};
    const author = window.getAuthorFromProfile ? window.getAuthorFromProfile(profile) : {
      name: profile.display_name || 'Anonymous', avatar: profile.avatar_url || '', verified_status: profile.verified_status || 'none'
    };
    ...
    if (mediaType === 'video' && mediaUrl) {
      const ytMatch = mediaUrl.match(/(?:embed\/|youtu\.be\/)([a-zA-Z0-9_-]{11})/);
      const thumb = ytMatch ? `https://img.youtube.com/vi/${ytMatch[1]}/hqdefault.jpg` : mediaUrl;
      mediaHtml = `<div style="width:100%;aspect-ratio:16/9;border-radius:0;overflow:hidden;background:#000;margin-top:8px;position:relative;">
    ...
    return `<article style="border-bottom:1px solid var(--brd);padding:14px 0;cursor:pointer;" onclick="window.location.href='index.html?post=${p.id}'">
      <div style="display:flex;align-items:center;gap:10px;">
        ...
        <div style="min-width:0;">
          <div style="display:flex;align-items:center;gap:4px;">
            <span style="font-weight:700;font-size:14px;">${escapeHtml(author.name)}</span>${badgeHtml}
          </div>
      ...
      <div style="display:flex;gap:16px;margin-top:10px;font-size:12px;color:var(--muted);">
        ...
        <span>❤️ ${fmtNum(p.like_count || p.likes)}</span>
      </div>
    </article>`;
  }
  ...
  // ─── DISCOVERY SCREEN (empty query / on focus) ─────────────
  async function renderDiscovery() {
    ...
    if (recent.length) {
      html += `<div class="discovery-section"><h3>🕒 Recent Searches</h3>` +
        recent.map(r => {
          ...
          const clickAction = isTag ? `window.Hashtags.goToHashtag('${escapeHtml(r.slice(1)).replace(/'/g,"\\'")}')` : `Search.runSearch('${escapeHtml(r).replace(/'/g,"\\'")}')`;
          return `<div class="recent-search-row"><span onclick="${clickAction}" style="flex:1;">${escapeHtml(r)}</span><span class="remove" onclick="event.stopPropagation();Search.removeRecent('${escapeHtml(r).replace(/'/g,"\\'")}')">×</span></div>`;
        }).join('') + `</div>`;
    }
    ...
    if (posts.length) {
      html += `<div class="discovery-section"><h3>📈 Trending Posts</h3>${posts.map(feedStylePostCard).join('')}</div>`;
    }
    ...
    if (videos.length) {
      html += `<div class="discovery-section"><h3>🎥 Trending Videos</h3><div style="display:flex;gap:8px;overflow-x:auto;padding-bottom:4px;">` +
        videos.map(v => {
          ...
          return `<div style="flex-shrink:0;width:110px;height:150px;border-radius:10px;overflow:hidden;position:relative;cursor:pointer;background:#000;" onclick="window.location.href='video.html?post=${v.id}'">
            <img src="${thumb||''}" style="width:100%;height:100%;object-fit:cover;">
    ...
    if (photos.length) {
      html += `<div class="discovery-section"><h3>📷 Trending Photos</h3><div class="square-grid">${photos.map(squareForPost).join('')}</div></div>`;
    }

    if (market.length) {
      html += `<div class="discovery-section"><h3>🛍 Trending Marketplace</h3><div style="display:flex;gap:10px;overflow-x:auto;padding-bottom:4px;">${market.map(marketCardHtml).join('')}</div></div>`;
    }

    c.innerHTML = html || emptyState('Nothing to discover yet');
  }

  // ─── HASHTAGS TAB (own discovery — full list, not mixed) ───
  ...
  async function renderSearchResults(q) {
    ...
    if (currentTab === 'hashtags') {
      c.innerHTML = hashtags.length
        ...
        : emptyState('No hashtags found', `No hashtags match "${q}"`);
      return;
    }
    if (currentTab === 'sounds') { renderSounds(); return; }

    // TOP — mixed feed, using normal post rendering (not text cards)
    let html = '';
    posts.slice(0, 8).forEach(p => { html += feedStylePostCard(p); });
    if (users.length) html += users.slice(0, 3).map(userRowHtml).join('');
    if (market.length) html += `<div style="display:flex;gap:10px;overflow-x:auto;padding:10px 0;">${market.slice(0, 4).map(marketCardHtml).join('')}</div>`;
    if (hashtags.length) html += hashtags.slice(0, 3).map(t => `<div class="hashtag-row" onclick="window.Hashtags.goToHashtag('${escapeHtml(t)}')"><span class="tag-name">#${escapeHtml(t)}</span></div>`).join('');
  ...
  async function loadHashtagResults() {
    ...
    document.getElementById('resultsContainer').innerHTML = posts.length
      ...
      : emptyState('No posts yet', `Be the first to post with #${currentHashtag}`);
  }
  ...
  function switchTab(tab) {
    ...
    if (currentQuery) { renderSearchResults(currentQuery); return; }
    if (tab === 'hashtags') { renderHashtagsTabDiscovery(); return; }
    renderDiscovery();
  }
  ...
  function onQueryInput(val) {
    ...
    if (!currentQuery) {
      if (currentTab === 'hashtags') renderHashtagsTabDiscovery();
  ...
  window.Search = {
    ...
    exitHashtagMode, setHashtagSort, removeRecent: removeRecentSearch, cancelFocus
  };

  // ─── INIT ───────────────────────────────────────────────────
  function init() {
    const params = new URLSearchParams(window.location.search);
    const tagParam = params.get('tag') || params.get('hashtag'); // back-compat
    if (tagParam) {
