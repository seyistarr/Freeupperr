...
(function () {
  ...
  // ─── DEDICATED SEARCH FUNCTIONS (optimized) ──────────────
  ...
  function squareForPost(p) {
    ...
    if (mediaType === 'video' && mediaUrl) {
      ...
      inner = ytMatch ? `<img src="${thumb}" loading="eager" decoding="async" draggable="false">` : `<video src="${mediaUrl}" muted playsinline preload="metadata" poster="${thumb}"></video>`;
      ...
      inner = `<img src="${mediaUrl}" loading="eager" decoding="async" draggable="false" onerror="this.parentElement.innerHTML='<div class=\\'square-text-preview\\' style=\\'background:#7C3AED\\'>${escapeHtml(p.title||'').replace(/'/g,"\\'")}</div>'">`;
    } else {
    ...
    return `<div class="square-item" onclick="window.Router.openPostById('${p.id}','${mediaType||''}')">${inner}</div>`;
  }
  ...
  function feedStylePostCard(p) {
    ...
    return `<article style="border-bottom:1px solid var(--brd);padding:14px 0;cursor:pointer;" onclick="window.Router.openPostById('${p.id}','${mediaType||''}')">
  ...
  function userRowHtml(u) {
    ...
    return `<div class="user-result" onclick="window.Router.openProfile('${u.id}')">
  ...
  function marketCardHtml(m) {
    ...
    return `<div style="flex-shrink:0;width:130px;cursor:pointer;" onclick="window.Router.openListing('${m.id}')">
  ...
  async function renderDiscovery() {
    ...
    html += `<div class="discovery-section"><h3>🔥 Trending Hashtags</h3>` +
      hashtags.map(h => `<div class="hashtag-row" onclick="Search.openHashtag('${escapeHtml(h.tag)}')"><span class="tag-name">#${escapeHtml(h.tag)}</span><span class="tag-count">${fmtNum(h.count)} posts</span></div>`).join('') + `</div>`;
    if (users.length) {
      html += `<div class="discovery-section"><h3>👤 Suggested Users</h3><div style="display:flex;gap:14px;overflow-x:auto;padding-bottom:4px;">` +
        users.map(u => `<div style="text-align:center;flex-shrink:0;cursor:pointer;width:64px;" onclick="window.Router.openProfile('${u.id}')">
    ...
    if (videos.length) {
      html += `<div class="discovery-section"><h3>🎥 Trending Videos</h3><div style="display:flex;gap:8px;overflow-x:auto;padding-bottom:4px;">` +
        videos.map(v => {
          ...
          return `<div style="flex-shrink:0;width:110px;height:150px;border-radius:10px;overflow:hidden;position:relative;cursor:pointer;background:#000;" onclick="window.Router.openPostById('${v.id}','video')">
            <img src="${thumb||''}" style="width:100%;height:100%;object-fit:cover;">
  ...
  async function renderSearchResults(q, replace = true) {
    ...
    if (currentTab === 'users') {
      ...
      html = renderSquareGrid(posts);
      if (!html) html = emptyState('No videos found', `No videos match "${q}"`);
    } else if (currentTab === 'photos') {
      const posts = await searchPhotos(q, tabState.offset);
      dataCount = posts.length;
      html = renderSquareGrid(posts);
      if (!html) html = emptyState('No photos found', `No photos match "${q}"`);
    } else if (currentTab === 'market') {
      const listings = await searchListings(q, tabState.offset);
      dataCount = listings.length;
      const cards = listings.map(m => window.renderListingCard ? window.renderListingCard(m) : marketCardHtml(m)).join('');
      ...
      html = hashtags.map(t => `<div class="hashtag-row" onclick="Search.openHashtag('${escapeHtml(t)}')"><span class="tag-name">#${escapeHtml(t)}</span></div>`).join('');
      if (!html) html = emptyState('No hashtags found', `No hashtags match "${q}"`);
    } else {
      // TOP — mixed feed
      ...
      html = mixed.map(entry => {
        ...
        if (entry.type === 'market') return `<div style="padding:10px 0;border-bottom:1px solid var(--brd);cursor:pointer;display:flex;gap:10px;align-items:center;" onclick="window.Router.openListing('${entry.item.id}')">
        ...
        if (entry.type === 'hashtag') return `<div class="hashtag-row" onclick="Search.openHashtag('${escapeHtml(entry.item)}')"><span class="tag-name">#${escapeHtml(entry.item)}</span></div>`;
        return '';
      }).join('');
  ...
  async function renderHashtagsTabDiscovery() {
    ...
    c.innerHTML = hashtags.length
      ? hashtags.map(h => `<div class="hashtag-row" onclick="Search.openHashtag('${escapeHtml(h.tag)}')"><span class="tag-name">#${escapeHtml(h.tag)}</span><span class="tag-count">${fmtNum(h.count)} posts</span></div>`).join('')
