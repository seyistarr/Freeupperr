...
// Rebuilt UI/UX: SVG icons only, no emojis, 3D fire icon for trending
// ============================================================
(function () {
  'use strict';
  ...
  // ─── ICONS ────────────────────────────────────────────────
  const ICON = {
    search: '<svg viewBox="0 0 24 24" width="19" height="19" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round"><circle cx="11" cy="11" r="8"/><line x1="21" y1="21" x2="16.65" y2="16.65"/></svg>',
    ...
    hashtag: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round"><line x1="9" y1="3" x2="6" y2="21"/><line x1="18" y1="3" x2="15" y2="21"/><line x1="3" y1="9" x2="21" y2="9"/><line x1="2.5" y1="15" x2="20.5" y2="15"/></svg>',
    ...
    dots: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round"><circle cx="12" cy="5" r="1.2"/><circle cx="12" cy="12" r="1.2"/><circle cx="12" cy="19" r="1.2"/></svg>',
    ...
    clock: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"><circle cx="12" cy="12" r="9"/><polyline points="12 7 12 12 15.5 14"/></svg>',
    ...
    // 3D fire — layered gradient flame with highlight for depth
    fire: `<svg viewBox="0 0 24 24" width="22" height="22">
      <defs>
      ...
      <path d="M12 2c1 3-2 4-2 7a3 3 0 1 0 6 0c0-1-1-2-1-3 2 1 4 4 4 7.5A6.5 6.5 0 0 1 12.5 20 6.5 6.5 0 0 1 6 13.5C6 8 10 5 12 2z" fill="url(#fireOuter)" filter="drop-shadow(0 2px 3px rgba(180,60,10,.35))"/>
  ...
  async function toggleFollowUser(userId, btn) {
    ...
    try {
      ...
      if (btn) { btn.textContent = relLabel(state); btn.className = 'rel-btn small rel-' + state; }
  ...
  function renderSquareGrid(posts) {
    ...
    return `<div class="sq-grid">${posts.map(squareForPost).join('')}</div>`;
  }
  function squareForPost(p) {
    ...
    if (mediaType === 'video' && mediaUrl) {
      ...
      inner += `<span class="vid-flag">${ICON.play}</span>`;
    } else if (mediaUrl) {
      inner = `<img src="${mediaUrl}" loading="lazy" draggable="false" onerror="this.parentElement.innerHTML='<div class=\\'sq-text-preview\\' style=\\'background:#7C3AED\\'>${escapeHtml(p.title||'').replace(/'/g,"\\'")}</div>'">`;
    } else {
  ...
  function feedCardHtml(p) {
    ...
    return `<div class="feed-card" onclick="window.Router.openPostById('${p.id}','${mediaType||''}')">
      <div class="thumb-wrap">
        <img src="${thumb||''}" loading="lazy">
        ${isVideo ? `<span class="dur">${p.duration||''}</span><span class="play-c">${ICON.play}</span>` : ''}
        <span class="dots-overlay">${ICON.dots}</span>
      </div>
  ...
  function postCardHtml(p) {
    ...
    const mediaUrl = p.media_url || p.mediaUrl;
    const thumb = getThumb(mediaUrl);
    return `<div class="post-card" onclick="window.Router.openPostById('${p.id}','${mediaType||''}')">
      <div class="head">
  ...
  function feedStylePostCard(p) {
    ...
    if (mediaUrl) {
      mediaHtml = `<div class="media" onclick="event.stopPropagation()">
        <img src="${thumb||''}" loading="lazy">
        ${isVideo ? `<span class="play-c">${ICON.play}</span>` : ''}
        <div class="stats">
          <span>${ICON.heart}${fmtNum(p.like_count || p.likes)}</span>
          ${isVideo ? `<span>${ICON.play}${fmtNum(p.views)}</span>` : ''}
        </div>
      </div>`;
    }
  ...
  function userRowHtml(u) {
    ...
    const btn = state === 'self' ? '' : `<button class="rel-btn small rel-${state}" onclick="_searchToggleFollow(event,'${u.id}')">${relLabel(state)}</button>`;
    return `<div class="user-row" onclick="window.Router.openProfile('${u.id}')">
      <div class="av"><img src="${u.avatar_url||''}" onerror="this.style.display='none'"></div>
      <div class="info">
        <div class="name-line">${escapeHtml(u.display_name||u.username||'')}${badgeHTML(u.verified_status)}</div>
  ...
  function marketCardHtml(m) {
    ...
    const condClass = cond.includes('new') && !cond.includes('like') ? 'cond-new' : cond.includes('like') ? 'cond-likenew' : 'cond-good';
    return `<div class="market-card" onclick="window.Router.openListing('${m.id}')">
      <div class="img-wrap">
        <img src="${img}" loading="lazy">
        ${m.featured ? `<span class="featured">Featured</span>` : ''}
        <span class="fav" onclick="event.stopPropagation()">${ICON.heart.replace('fill="currentColor"','fill="none" stroke="#111" stroke-width="2"')}</span>
      </div>
      <div class="title">${escapeHtml(m.title||'')}</div>
      <div class="price">₦${Number(m.price||0).toLocaleString()}</div>
      <div class="meta-row">
        <span class="loc">${ICON.pin}${escapeHtml(m.location||'')}</span>
        ${m.condition ? `<span class="cond ${condClass}">${escapeHtml(m.condition)}</span>` : ''}
      </div>
    </div>`;
  }
  ...
  function showTabs(show) {
    ...
    if (tabs) tabs.classList.toggle('visible', show);
  }
  ...
  function renderTabsBar() {
    ...
    document.getElementById('searchTabs').innerHTML = defs.map(t =>
      `<button class="tab${t.id===currentTab?' active':''}" data-tab="${t.id}" onclick="Search.switchTab('${t.id}')">${t.icon}<span>${t.label}</span></button>`
    ).join('');
  }
  ...
  async function renderDiscovery() {
      ...
      recentHtml = `<div class="discovery-block">
        ...
        <div class="recent-wrap">${recent.map(r => {
          ...
          return `<div class="recent-pill">
            <span class="clock" style="display:flex;">${ICON.clock}</span>
    ...
    if (posts.length) html += `<div class="discovery-block"><div class="section-title">Trending Posts</div>${posts.map(feedStylePostCard).join('')}</div>`;
    ...
    if (videos.length) {
      html += `<div class="discovery-block"><div class="section-title">${ICON.video}Trending Videos</div>
        <div class="sugg-scroll">${videos.map(v => {
          ...
          return `<div style="flex-shrink:0;width:118px;">
            <div style="position:relative;width:118px;height:160px;border-radius:14px;overflow:hidden;background:#000;cursor:pointer;" onclick="window.Router.openPostById('${v.id}','video')">
              <img src="${thumb||''}" style="width:100%;height:100%;object-fit:cover;">
              <span class="page-badge" style="position:absolute;bottom:8px;left:8px;background:none;padding:0;color:#fff;font-weight:700;font-size:11px;text-shadow:0 1px 4px rgba(0,0,0,.7);">${fmtNum(v.views)} views</span>
            </div>
          </div>`;
        }).join('')}</div></div>`;
    }

    if (photos.length) html += `<div class="discovery-block"><div class="section-title">${ICON.photo}Trending Photos</div>${renderSquareGrid(photos)}</div>`;

    if (market.length) html += `<div class="discovery-block"><div class="section-title">${ICON.market}Trending Marketplace</div>
      <div class="sugg-scroll">${market.map(m => `<div style="flex-shrink:0;width:130px;cursor:pointer;" onclick="window.Router.openListing('${m.id}')">
        <div style="width:130px;height:130px;border-radius:14px;overflow:hidden;background:var(--bg4);"><img src="${(m.images&&m.images[0]&&m.images[0].url)||''}" style="width:100%;height:100%;object-fit:cover;"></div>
        <div style="font-size:13px;font-weight:800;margin-top:6px;color:var(--p);">₦${Number(m.price||0).toLocaleString()}</div>
        <div style="font-size:11.5px;color:var(--muted);overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">${escapeHtml(m.title||'')}</div>
      </div>`).join('')}</div></div>`;
  ...
  // ─── LIVE SEARCH ─────────────────────────────────
  const TAB_LABEL = { top: 'Results', users: 'Users', videos: 'Videos', photos: 'Photos', market: 'Market', hashtags: 'Hashtags' };
  ...
  async function renderSearchResults(q, replace = true) {
    ...
    if (currentTab === 'users') {
      ...
      html = users.map(userRowHtml).join('');
      ...
      html = posts.length ? `<div class="card-grid">${posts.map(feedCardHtml).join('')}</div>` : emptyState('No videos found', `No videos match "${q}"`);
      if (replace) headHtml = `<div class="results-head"><div><h2 style="margin-bottom:2px;">Videos</h2><div style="color:var(--muted);font-size:13px;">Top videos matching "${escapeHtml(q)}"</div></div><div class="sort-dd">Sort: Top ${ICON.chevDown}</div></div>`;
    } else if (currentTab === 'photos') {
      const posts = await searchPhotos(q, tabState.offset);
      dataCount = posts.length;
      html = renderSquareGrid(posts) || emptyState('No photos found', `No photos match "${q}"`);
      ...
      html = cards ? `<div class="grid-2">${cards}</div>` : emptyState('No listings found', `No listings match "${q}"`);
      ...
      html = mixed.map(entry => {
        ...
        if (entry.type === 'market') return marketCardHtml(entry.item);
    ...
    createSentinel();
    if (!tabState.hasMore) showEndResults(); else showLoadingMore(false);
    startInfiniteScroll();
  }
    ...
    hh.innerHTML = `
      <div class="top-row">
        ...
        <div class="side-actions">
          <button class="rel-btn small">Follow</button>
          <button class="icon-btn">${ICON.share}</button>
        </div>
      </div>
      <div class="subtabs">
        <button data-sort="top" class="active" onclick="Search.setHashtagSort('top')">${ICON.search}Top</button>
        <button data-sort="photos" onclick="Search._setHashtagView('photos')">${ICON.photo}Photos</button>
        <button data-sort="videos" onclick="Search._setHashtagView('videos')">${ICON.video}Videos</button>
      </div>`;
  }

  let hashtagView = 'top'; // top | photos | videos

  window.Search_setHashtagView = null; // placeholder (kept internal via public API below)
  ...
  function setHashtagView(view) {
    hashtagView = view;
    document.querySelectorAll('#hashtagHeader .subtabs button').forEach(b => b.classList.toggle('active', b.dataset.sort === view || (view==='top' && b.dataset.sort==='top')));
  ...
  async function loadHashtagResults(replace = true) {
    ...
    if (hashtagView === 'photos') {
      html = renderSquareGrid(posts.filter(p => (p.media_type||p.mediaType) !== 'video'));
    } else if (hashtagView === 'videos') {
      html = posts.length ? `<div class="card-grid">${posts.filter(p=>(p.media_type||p.mediaType)==='video').map(feedCardHtml).join('')}</div>` : '';
    } else {
      html = `<div class="card-grid">${posts.map(postCardHtml).join('')}</div>`;
    }
  ...
  function setHashtagSort(sort) {
    hashtagSort = sort;
    hashtagView = 'top';
    document.querySelectorAll('#hashtagHeader .subtabs button').forEach(b => b.classList.toggle('active', b.dataset.sort === 'top'));
    paging.hashtags.offset = 0; paging.hashtags.hasMore = true; paging.hashtags.cache = null;
    document.getElementById('resultsContainer').innerHTML = '';
    loadHashtagResults(true);
  }
  ...
  // ─── INIT ───────────────────────────────────────────────────
  function init() {
    document.querySelector('.search-bar .ic-left').innerHTML = ICON.search;
    ...
    window.addEventListener('popstate', () => {
      ...
      else { hashtagMode = false; document.getElementById('hashtagHeader').style.display = 'none'; renderDiscovery(); }
    });
  }
